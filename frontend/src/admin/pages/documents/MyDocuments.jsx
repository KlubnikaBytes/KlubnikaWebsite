...
const DOCUMENT_CATEGORIES = [
    { type: 'Profile Photo',              icon: '🖼️',  multiple: false, desc: 'Passport-size photo or any professional headshot' },
    { type: 'Aadhaar Card',               icon: '🪪',  multiple: false, desc: 'Front and back side in one file' },
    { type: 'PAN Card',                   icon: '💳',  multiple: false, desc: 'PAN card issued by Income Tax Department' },
    { type: 'Bank Details Document',      icon: '🏦',  multiple: false, desc: 'Passbook front page or cancelled cheque' },
    { type: '10th Marksheet',             icon: '📄',  multiple: false, desc: 'Class X board marksheet' },
    { type: '12th Marksheet',             icon: '📄',  multiple: false, desc: 'Class XII board marksheet' },
    { type: 'Semester Results',           icon: '📊',  multiple: true,  desc: 'Upload each semester result separately' },
    { type: 'Degree Certificate',         icon: '🎓',  multiple: false, desc: 'Final degree / provisional certificate' },
    { type: 'Experience Proof',           icon: '💼',  multiple: false, desc: 'Offer letter, experience letter, or relieving letter' },
    { type: 'Internship Certificate',     icon: '📋',  multiple: false, desc: 'Any internship completion certificate' },
];
                                            ...
                                            <div key={doc._id} className="rounded-xl border border-gray-100 bg-gray-50/50 p-3 space-y-2">
                                                ...
                                                <div className="flex gap-2 pt-1">
                                                    <button
                                                        ...
                                                        className="flex-1 text-xs font-semibold bg-white border border-gray-200 text-gray-600 hover:text-indigo-700 hover:border-indigo-200 hover:bg-indigo-50 py-1.5 rounded-lg transition-colors"
                                                    >
                                                        👁 View
                                                    </button>
                                                    {doc.status === 'pending' && (
                                                        <button
                                                            ...
                                                            className="text-xs font-semibold bg-white border border-gray-200 text-rose-500 hover:text-rose-700 hover:border-rose-200 hover:bg-rose-50 px-3 py-1.5 rounded-lg transition-colors"
                                                        >
                                                            🗑
                                                        </button>
                                                    )}
                            ...
                            <div className="px-5 py-3.5 border-t border-gray-50 bg-gray-50/30">
                                ...
                                >
                                    {isUploading ? (
                                        ...
                                        <>{catDocs.length > 0 && !cat.multiple ? '🔄 Replace' : '⬆ Upload'}</>
                                    )}
                                </button>
                            </div>
                        </div>
            ...
            {hrUploadedDocs.length > 0 && (
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest">📂 Documents from HR / Company</span>
                        <div className="h-px flex-1 bg-gray-100" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                            ...
                            return (
                                <div key={doc._id} className="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden flex flex-col">
                                    <div className="px-5 py-4 border-b border-indigo-50 bg-indigo-50/50 flex items-center gap-3">
                                        <span className="text-2xl">{doc.mimeType === 'application/pdf' ? '📑' : '🖼️'}</span>
                                        <div className="flex-1 min-w-0">
                                        ...
                                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 border border-indigo-200 shrink-0">🏢 Company</span>
                                    </div>
